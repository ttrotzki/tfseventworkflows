...
namespace artiso.TFSEventWorkflows.TFSActivitiesLib.Workspace_Activities
{
    ...
    public sealed class AddToSourceControl : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
          ...
          catch (CheckinException ciex)
          {
            System.Text.StringBuilder sbMessage = new System.Text.StringBuilder();
            sbMessage.AppendLine("Activity AddToSourceControl:");
            foreach (var conflict in ciex.Conflicts)
            {
              sbMessage.AppendFormat("conflict resolveable: \"{0}\" serveritem: \"{1}\" message: \"{2}\"", 
                conflict.Resolvable, conflict.ServerItem, conflict.Message);
              sbMessage.AppendLine();
            }
            throw new Exception(sbMessage.ToString(), ciex);
          }
          catch (Exception ex)
          {
